...
# parameters:
noise = np.random.normal(11, 2, size=int(length/interval))
...
adv_force = np.random.normal(10000, 2000, size=int(length/interval))
...
speed = (penetration * rpm) / 1000

noise = np.random.normal(2, 0.2, size=int(length/interval))
torque = np.cos(orig)
...
torque = np.where(torque > 4.5, np.random.normal(2, 0.2), torque)
